...
function initTools() {
  ...
  tools.forEach((tool, index) => {
    ...
    card.className = 'card tool-card shadow-sm h-100';
    card.setAttribute('data-tool-id', tool.id);
    card.style.cursor = 'pointer';
    card.innerHTML = `
      <div class="card-body">
        <div class="d-flex align-items-center justify-content-between mb-3">
          <i class="bi ${tool.icon} ${tool.color} tool-icon"></i>
        </div>
...
const timerMessages = [
  ...
  'Cada minuto conta! Mantenha o ritmo! ⏰',
...
function resetTimer() {
  ...
  const btn = document.getElementById('timerPlayPause');
  if (btn) {
    btn.innerHTML = '<i class="bi bi-play-fill me-2"></i>Iniciar';
  }
...
function showTimerMessage() {
  const message = timerMessages[Math.floor(Math.random() * timerMessages.length)];
  const messageDiv = document.getElementById('timerMessage');
  if (messageDiv) {
...
function completeTimer() {
  resetTimer();
  alert('🎉 Parabéns! Você completou o desafio! Continue assim!');
}
