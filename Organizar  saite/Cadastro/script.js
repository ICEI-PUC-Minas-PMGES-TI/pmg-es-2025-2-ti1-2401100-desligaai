    ...
    async register(userData) {
        try {
            ...
            const newUser = {
                ...
                level: 1,
                theme: 'light',
                // Campos obrigatórios para o sistema pós-login
                currentDay: 1,
                rank: 'Bronze',
            ...
            if (!loginResult.success) {
                // Se o login automático falhou, retornar sucesso mas sem login
                console.warn('[REGISTER] Login automático falhou:', loginResult.message);
                return {
                    success: true,
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Verificar se já está logado - apenas redirecionar se realmente estiver logado
    // Usar sessionStorage para evitar loops infinitos
    const isRedirecting = sessionStorage.getItem('redirectingToLogin');
    const isAuthenticated = sessionStorage.getItem('isAuthenticated');
    
    // Só redirecionar se estiver autenticado E não estiver em processo de redirecionamento
    if (!isRedirecting && (isAuthenticated || authManager.isLoggedIn())) {
        const currentPath = window.location.pathname || window.location.href;
        // Só redirecionar se estiver na página de login e não estiver em processo de redirecionamento
        if (currentPath.includes('login.html')) {
            sessionStorage.removeItem('stayOnLogin'); // Limpar flag se existir
            // Redirecionar para página pós-login (index.html) se estiver logado
            setTimeout(() => {
                const redirectPath = '../PAGINA POS LOGIN/index.html';
                window.location.replace(redirectPath);
            }, 100);
        }
    }
    
    // Limpar flag de redirecionamento após carregamento
    if (isRedirecting) {
    ...
    loginForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (result.success) {
                ...
                console.log('[LOGIN] Usuário salvo no localStorage:', localStorage.getItem('desligaAI_currentUser') ? 'Sim' : 'Não');
                ...
                // Verificar se o usuário foi salvo corretamente antes de redirecionar
                const savedUser = localStorage.getItem('desligaAI_currentUser');
                if (!savedUser) {
                    console.error('[LOGIN] ERRO: Usuário não foi salvo no localStorage!');
                    showMessage('❌ Erro ao salvar sessão. Tente novamente.', 'error');
                    submitBtn.innerHTML = originalText;
                    submitBtn.disabled = false;
                    return;
                }
                
                console.log('[LOGIN] Usuário confirmado no localStorage, redirecionando...');
                
                // Limpar flags de redirecionamento antes de redirecionar
                sessionStorage.removeItem('redirectingToLogin');
                sessionStorage.removeItem('stayOnLogin');
                // Marcar que acabou de fazer login para ajudar na verificação
                sessionStorage.setItem('justLoggedIn', 'true');
                // Marcar que está autenticado para evitar loop
                sessionStorage.setItem('isAuthenticated', 'true');
                
                // Redirecionar após 1.5 segundos para a página pós-login (index.html)
                // Usar replace para evitar problemas de histórico e loops
                setTimeout(() => {
                    // Usar caminho relativo correto com encodeURIComponent para espaços
                    const redirectPath = '../PAGINA POS LOGIN/index.html';
                    
                    console.log('[LOGIN] Tentando redirecionar para página pós-login...');
                    console.log('[LOGIN] Redirecionando para:', redirectPath);
                    
                    // Redirecionar diretamente para index.html
                    window.location.replace(redirectPath);
                }, 1500);
            } else {
...
function setupSignupForm() {
    ...
    async function submitSignup() {
        ...
        try {
            ...
            if (result.success) {
                // Verificar se o login automático funcionou
                const savedUser = localStorage.getItem('desligaAI_currentUser');
                
                if (savedUser && !result.autoLoginFailed) {
                    // Login automático funcionou, redirecionar para página pós-login
                    showMessage('✅ Conta criada e login realizado com sucesso!', 'success');
                    sessionStorage.setItem('justLoggedIn', 'true');
                    sessionStorage.setItem('isAuthenticated', 'true');
                    setTimeout(() => {
                        window.location.replace('../PAGINA POS LOGIN/index.html');
                    }, 1500);
                } else {
                    // Login automático falhou, redirecionar para login
